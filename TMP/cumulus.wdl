...
import "https://raw.githubusercontent.com/lilab-bcb/cumulus/master/workflows/cumulus/cumulus.wdl" as cmls
...
workflow cumulus {
    ...
    call cmls.cumulus as cumulus {
        input:
            ...
            calc_signature_scores = calc_signature_scores,
            nPC = nPC,
            run_nmf = run_nmf,
            nmf_n = nmf_n,
            knn_K = knn_K,
            ...
            generate_cirro_inputs = generate_cirro_inputs,
            generate_scp_outputs = generate_scp_outputs,
            output_dense = output_dense
    }

    output {
        File? output_aggr_zarr = cumulus.output_aggr_zarr
        File output_zarr = cumulus.output_zarr
        File output_cluster_log = cumulus.output_cluster_log
        Array[File] output_h5ad = cumulus.output_h5ad
        Array[File] output_filt_xlsx = cumulus.output_filt_xlsx
        Array[File] output_filt_plot = cumulus.output_filt_plot
        Array[File] output_hvf_plot = cumulus.output_hvf_plot
        Array[File] output_dbl_plot = cumulus.output_dbl_plot
        Array[File] output_loom_file = cumulus.output_loom_file
        Array[File?]? output_de_h5ad = cumulus.output_de_h5ad
        Array[File?]? output_de_xlsx = cumulus.output_de_xlsx
        Array[Array[File]?]? output_markers_xlsx = cumulus.output_markers_xlsx
        Array[Array[File]?]? output_anno_file = cumulus.output_anno_file
        Array[Array[File]?]? output_pdfs = cumulus.output_pdfs
        Array[Array[File]?]? output_htmls = cumulus.output_htmls
        Array[String?]? output_cirro_folder = cumulus.output_cirro_folder
        Array[Array[File]?]? output_scp_files = cumulus.output_scp_files
    }
}
